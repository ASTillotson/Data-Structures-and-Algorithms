...
public class StreetMapGraph implements AStarGraph<Long> {
    ...
    private Map<String, Node> stringsToNodes;
    private Map<Point, Node> nodesToPoints;
    private autocomplete.BinaryRangeSearch BRS;
    ...
    public StreetMapGraph(String filename) {
        ...
        for (Long key : this.nodes.keySet()) {
            //initializing the list of points and the map that maps points to nodes for easy lookup later
            double x = projectToX(nodes.get(key).lon(), nodes.get(key).lat());
            double y = projectToY(nodes.get(key).lon(), nodes.get(key).lat());
            if (this.isNavigable(this.nodes.get(key))) {
                Point temp = new Point(x, y);
                points.add(temp);
                nodesToPoints.put(temp, nodes.get(key));
            }
            //initializing the term array for the autocomplete Binary range search
            if (nodes.get(key).name() != null) {
                Term theTerm = new Term(nodes.get(key).name(), nodes.get(key).importance);
                terms.add(theTerm);
            }
            //initializing the string to node map
            stringsToNodes.put(nodes.get(key).name(), nodes.get(key));
        }
        kdTree = new KDTreePointSet(points);
        Term[] theTerms = new Term[terms.size()];
        for (int i = 0; i < terms.size(); i++) {
            theTerms[i] = terms.get(i);
        }
        BRS = new BinaryRangeSearch(theTerms);
    }
    ...
    public long closest(double lat, double lon) {
        ...
        long id = nodesToPoints.get(nearest).id;
        return id;
    }
    ...
    public List<String> getLocationsByPrefix(String prefix) {
        Term[] ans = BRS.allMatches(prefix);
        List<String> listAns = new ArrayList<>();
        for (int i = 0; i < ans.length; i++) {
            listAns.add(ans[i].query());
        }
        return listAns;
    }
    ...
    public List<Node> getLocations(String locationName) {
        Term[] ans = BRS.allMatches(locationName);
        List<Node> listAns = new ArrayList<>();
        for (int i = 0; i < ans.length; i++) {
            listAns.add(stringsToNodes.get(ans[i].query()));
        }
        return listAns;
    }
